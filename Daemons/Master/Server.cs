...
namespace Minerva
{
	class Server
	{
		...
		public Server()
		{
			Console.Title = ".:: Minerva - Cabal Online Sandbox - Master Server ::.";
			...
			try
			{
				...
				foreach (var n in node.Elements("init"))
					stats.Add(int.Parse(n.Attribute("style").Value), new List<int>() { int.Parse(n.Attribute("world_id").Value),
				...
				foreach (var n in node.Elements("initstat"))
					stats[int.Parse(n.Attribute("id").Value)].AddRange(new[] { int.Parse(n.Attribute("str").Value),
				...
				var items = new Dictionary<int, List<Tuple<byte[], int, byte>>>();
				...
				foreach (var n in node.Elements("item"))
				{
					...
					if (!items.ContainsKey(_class))
						items[_class] = new List<Tuple<byte[], int, byte>>();

					items[_class].Add(Tuple.Create(item.ToByteArray(), amount, slot));
				}
